    ...
    def plot_simulation_state(self, model, current_state, chosen_edge=None):
        ...
        if chosen_edge is not None:
            ...
            if (dep, dest) in G.edges():
                ...
                base_offset = 0.33
                # simple approach: just highlight the first one
                # or find the one that matches t_type if you want
                nx.draw_networkx_edges(
...
class MainWindow(QMainWindow):
    ...
    def probability_analysis(self):
        if not self.model:
            ...
            return
        # Prompt the user for win states
        text, ok = QInputDialog.getText(self, "Win States", "Enter win state(s), comma-separated:")
        if not ok:
            return
        ...
        print(f"Win: {w}")
        print(f"Lose: {l}")
        print(f"Incertitude: {inc}")
        # Probability (MC only, but you can adapt)
        if inc:
            probs = self.model.calcul_proba_MC(w, inc, self.model.transitions)
            print("Probabilities of eventually reaching a win state from incertitude states:")
            for st, pr in zip(inc, probs):
                print(f"   {st}: {pr:.4f}")
        else:
    ...
    def simulate_step(self):
        ...
        if len(outgoing) == 1 and outgoing[0][3] == self.current_state:
            print(f"[SIM] {self.current_state} in a loop => stopping.")
            self.stop_simulation()
            return
    ...
    def print_matrix(self):
        ...
        print("\nTransition Matrix (each row = one transition):")
        print("Columns =>", cols)
